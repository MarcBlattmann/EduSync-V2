...
export function ShareNoteDialog({
  ...
  const [shareName, setShareName] = useState(`${noteTitle || "My Note"}`);
  ...
  const fetchShares = async () => {
    ...
    try {
      setIsLoading(true);
      setError(null);
      const response = await fetch(`/api/note-share?noteId=${noteId}`);
  ...
  useEffect(() => {
    if (open && noteId) {
      fetchShares();
      setShareName(`${noteTitle || "My Note"}`);
    }
  }, [open, noteId, noteTitle]);
  ...
  const handleCreateShare = async () => {
    ...
    try {
      ...
      setError("Failed to create note share");
    } finally {
  ...
  const handleDeleteShare = async (shareId: string) => {
    try {
      ...
      const response = await fetch(`/api/note-share?id=${shareId}`, {
        method: "DELETE",
      });
      ...
      setError("Failed to delete note share");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCopyLink = (shareCode: string) => {
    const url = `${window.location.origin}/notes/${shareCode}`;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a link that lets others view your note
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="mt-2">
            <Button
              ...
              disabled={!shareName.trim() || creatingShare || !noteId}
              className="w-full"
            >
